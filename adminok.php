	...
	case "forum":
		...
		case "creer":
			...
			if ($_GET['c'] == "f")
			{
				...
				$query=$bd->prepare('INSERT INTO forum_forum (forum_cat_id, forum_name, forum_desc) 
			...
			{
				$titre = $_POST['nom'];
				$query=$bd->prepare('INSERT INTO forum_categorie (cat_nom) VALUES (:titre)');
				$query->bindValue(':titre',$titre, PDO::PARAM_STR); 
				$query->execute();          
		...
		case "edit":
			...
			elseif($_GET['e'] == "editc")
			{
				...
				//Mise à jour
				$query=$bd->prepare('UPDATE forum_categorie
			...
			elseif($_GET['e'] == "ordref")
			{
				...
				$query=$bd->query('SELECT forum_id, forum_ordre FROM forum_forum');
				...
				while($data= $query->fetch())
				{
					...
					{
						$query=$bd->prepare('UPDATE forum_forum SET forum_ordre = :ordre
						WHERE forum_id = :id');
