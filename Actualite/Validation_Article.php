 ...
 */
	...
	if($_SESSION['categorie']!='moderateur' or $page<0)
		echo '<script>
 ...
 */
	...
	$request = 'SELECT count(*) FROM Article WHERE validation=0'; /* La requête SQL */
 ...
 */
					$req = "SELECT id_article, titre, DATE_FORMAT(jour,'%d %b %Y %T'), auteur, corps FROM Article WHERE validation = 0 ORDER BY id_article DESC LIMIT 10 OFFSET $limiter";
